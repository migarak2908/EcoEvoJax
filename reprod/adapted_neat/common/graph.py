...
from reprod.adapted_neat.common.tools import fetch_first, I_INF
...
def topological_sort_python(
    ...
    # a python version of topological_sort, use python set to store node and conns
    # returns the topological order of the node and the topological layers
    # written by gpt4 :)

    # Make a copy of the input node and connections
    nodes = nodes.copy()
    conns = conns.copy()
    ...
    while zero_in_degree_nodes:
        ...
        for node in zero_in_degree_nodes:
            ...
            # Iterate over all connections and reduce the in-degree of connected node
...
def find_useful_nodes(
    ...
    """
    Find all useful node (really contribute to outputs)
    """
    useful_nodes = set()
    ...
    while True:
        ...
        for in_, out in conns:
            if out in useful_nodes and in_ not in useful_nodes:
                aux.add(in_)
        if len(aux) == 0:  # no new node
            break
        else:
            useful_nodes = useful_nodes | aux
    # print(f"All node cnt={len(node)}, useful node cnt={len(useful_nodes)}")
    return useful_nodes
...
def check_cycles(nodes: Array, conns: Array, from_idx, to_idx) -> Array:
    ...
    def cond_func(carry):
        ...
        end_cond1 = jnp.all(visited_ == new_visited_)  # no new node been visited
